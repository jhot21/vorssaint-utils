...
Meeting notifications, when turned on, scan each event's URL, location and description text for a Zoom, Google Meet or Microsoft Teams link — this is the first feature to read an event's description, used only to find a link, held only in memory, and never transmitted anywhere. A scheduled notification's local payload (delivered through the system's own notification center, never sent off this Mac) carries the resolved meeting link so it can still be opened if you tap the notification before the app has finished re-reading your calendar, such as right after a fresh launch. When enabled, joining a Google Meet link opens it in the browser you choose in Settings instead of always using your system default.

The optional Next Meeting menu bar reading shows the provider and minutes remaining for the same soon-starting event meeting notifications already detect a link for — no additional calendar data is read for it, but the reading is visible in the menu bar itself, so anyone glancing at your screen can see that a meeting is coming up.
